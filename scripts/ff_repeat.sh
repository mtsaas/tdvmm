...
# trim behavior (the emitted DVMM_DEMO_SEQ must match byte-for-byte).
#
...
# DVMM_DEMO_SEQ line on stdout (so the caller compares sequences, not transcripts
...
run() {
  ...
  echo "$out" | grep -oE 'DVMM_DEMO_SEQ=.*' | head -1
}
