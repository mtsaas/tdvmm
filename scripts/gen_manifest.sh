...
INITRD="$ROOT/guest/initramfs-alpine/initramfs-alpine.cpio.gz"
...
[ -f "$INITRD" ]  || { echo "gen_manifest: initrd not found: $INITRD" >&2; exit 3; }
if [ ! -x "$BIN" ]; then
...
initramfs  sha256=$IN_SHA  guest/initramfs-alpine/initramfs-alpine.cpio.gz
