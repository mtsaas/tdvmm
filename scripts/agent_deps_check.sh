#!/usr/bin/env bash
# Assert `cargo tree -p tdvmm-agent` matches the committed allowlist (Fable §1a).
#
# The guest agent's dependency closure must stay minimal: tdvmm-proto + the
...
ALLOW="$ROOT/tdvmm-agent/deps.allow"

[ -f "$ALLOW" ] || { echo "FAIL: missing allowlist $ALLOW"; exit 1; }

got="$(cd "$ROOT" && cargo tree -p tdvmm-agent --prefix none 2>/dev/null \
        | sed -E 's/ v[0-9].*//; s/ \(.*//' | awk 'NF' | sort -u)"
want="$(sort -u "$ALLOW")"

echo "== tdvmm-agent dependency allowlist gate =="
if ! diff <(printf '%s\n' "$want") <(printf '%s\n' "$got"); then
  echo "FAIL: tdvmm-agent dep tree deviates from $ALLOW (diff above:"
  echo "      '<' = allowed-but-absent, '>' = present-but-not-allowed)."
  echo "      If intended, regenerate: cargo tree -p tdvmm-agent --prefix none \\"
...
if printf '%s\n' "$got" | grep -Eq "^($forbidden)$"; then
  echo "FAIL: a forbidden crate leaked into tdvmm-agent's dep tree"; exit 1
fi

echo "PASS: tdvmm-agent dep tree == allowlist ($(printf '%s\n' "$got" | wc -l) crates; no heavy deps)"
