#!/usr/bin/env bash
# Agent DOUBLE-BUILD byte-identity gate (Fable §3): build the dvmm-agent twice in
# FRESH pinned builder containers and require an identical sha256. Proves the musl
# build is reproducible (the property the `.dvmm` bit-repro guarantee rests on for
# the baked agent).
#
...
echo "== dvmm-agent double-build byte-identity gate =="
