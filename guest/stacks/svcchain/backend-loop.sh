...
echo "DVMM_BACKEND_START host=$PGHOST db=$PGDATABASE interval=${INTERVAL_SECONDS}s max_rows=$MAX_ROWS"
until pg_isready -q -h "$PGHOST" -U "$PGUSER"; do
  echo "DVMM_BACKEND_WAIT db not ready yet"; sleep 1
done
...
touch /tmp/ready
echo "DVMM_BACKEND_READY connected to db + schema ensured"
