#!/bin/sh
# tdvmm corpus (configpipeline) worker service.
#
...
echo "TDVMM_WORKER_START interval=${INTERVAL_SECONDS}s"
# (a) materialized RW bind: read the baked seed token.
echo "TDVMM_CONFIG_SEED=$(grep '^seed=' "$CFG" 2>/dev/null | cut -d= -f2)"
# (b) RW bind write + read back.
echo "generated-by-worker" > /etc/app/generated.txt 2>/dev/null
echo "TDVMM_CONFIG_WRITE_OK=$(cat /etc/app/generated.txt 2>/dev/null)"
...
while :; do
  ...
  echo "TDVMM_STATE_WRITE_OK iter=$i wrote=$(cat "$STATE" 2>/dev/null) ts=$(date -u '+%H:%M:%S')"
