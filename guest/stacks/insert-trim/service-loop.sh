#!/bin/sh
# tdvmm insert-trim service (compose path).
#
...
#
#   TDVMM_ROWCOUNT=<n> iter=<i> max=<MAX_ROWS> ...
#
...
echo "TDVMM_SVC_START host=$PGHOST db=$PGDATABASE interval=${INTERVAL_SECONDS}s max_rows=$MAX_ROWS"

# 1. Wait for Postgres to accept connections (retry loop over pg_isready).
until pg_isready -q -h "$PGHOST" -p "$PGPORT" -U "$PGUSER"; do
  echo "TDVMM_SVC_WAIT postgres not ready yet"
  sleep 1
done
echo "TDVMM_SVC_PG_READY"
...
then
  echo "TDVMM_SVC_FAIL create-table"
  exit 1
fi
...
while :; do
  ...
  then
    echo "TDVMM_SVC_FAIL insert iter=$i"
    exit 1
  fi
  ...
  then
    echo "TDVMM_SVC_FAIL trim iter=$i"
    exit 1
  fi

  n="$(psql -tAX -v ON_ERROR_STOP=1 -c 'SELECT count(*) FROM events;')"
  if [ -z "$n" ]; then
    echo "TDVMM_SVC_FAIL count iter=$i"
    exit 1
  fi
