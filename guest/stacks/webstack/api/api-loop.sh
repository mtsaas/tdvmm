#!/bin/sh
# tdvmm corpus (webstack) api service.
#
...
echo "TDVMM_API_UP both deps healthy: pg=$PGHOST redis=$REDIS_HOST interval=${INTERVAL_SECONDS}s max_rows=$MAX_ROWS"
...
if pg_isready -q -h "$PGHOST" -U "$PGUSER"; then echo "TDVMM_API_PG_OK"; else echo "TDVMM_API_PG_FAIL"; fi
PONG="$(redis-cli -h "$REDIS_HOST" ping 2>/dev/null)"
echo "TDVMM_API_REDIS_PING=${PONG:-none}"
...
while :; do
  ...
  echo "TDVMM_ROWCOUNT=$n iter=$i max=$MAX_ROWS cache_requests=${hits:-?} ts=$(date -u '+%Y-%m-%dT%H:%M:%SZ')"
