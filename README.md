...
tdvmm is a small KVM hypervisor (a VMM) written in Rust. It bakes a Docker
...
- **It fast-forwards idle time.** When the guest goes to sleep waiting for a
  timer, tdvmm jumps its clock straight to the next thing that happens. A service
  that inserts a row "every hour" runs its whole day in seconds.
- **A whole stack is one file.** `tdvmm build` bakes your compose stack — kernel,
  container images, and all — into a single, self-contained `.tdvmm` you can pin,
  share, and re-test offline. Identical inputs produce the byte-identical `.tdvmm`
...
- **To run** a `.tdvmm`: nothing else — just `/dev/kvm`.
- **To build** a `.tdvmm`: `podman` (plus network access the first time, to
...
```sh
cargo build --release        # produces ./target/release/tdvmm
```

tdvmm is a single self-contained binary. (A fully static release build is wired up
in `.github/workflows/release.yml`, triggered by a version tag.)

## How it works (the short version)

- **`tdvmm build <compose.yml>`** turns your stack into a `.tdvmm`. It resolves
...
- **`tdvmm run <stack>`** boots that stack in a VM: one virtual CPU, a serial
  console, and a clock and interrupt controller that tdvmm owns in userspace —
  which is what lets it fast-forward idle time. Fully offline.
- **`tdvmm test <stack> --scenario s.yml`** drives a timeline against the
...
`tdvmm ls` lists what you've built; `tdvmm inspect` prints what's inside a stack;
`tdvmm verify` checks it hasn't changed. A `.tdvmm` file's identity is just its
sha256.
...
tdvmm build guest/stacks/insert-trim/compose.yml   # -> ~/.tdvmm/artifacts/insert-trim.tdvmm

# List what you've built:
tdvmm ls                # names, sizes, when built
tdvmm ls --digest       # also compute each artifact's sha256 identity

# Run it by name (offline, only needs /dev/kvm). Idle time fast-forwards automatically:
tdvmm run insert-trim --max-virtual-time 24h
...
# Look at an artifact:
tdvmm inspect insert-trim    # its manifest
tdvmm verify  insert-trim    # check every piece matches, and print its sha256
```

`run`, `test`, `inspect`, and `verify` take a bare stack **name**, resolved from
your `~/.tdvmm/artifacts/` store the way `tdvmm build` names it. To point at a `.tdvmm`
file on disk instead, give a path (anything with a `/` — `./foo.tdvmm` or an absolute
...
`tdvmm test` exit codes: **0** all assertions passed, **1** an assertion or
...
scenario for `tdvmm test`). Start with **`demo`** — the capability reference.
...
```
$ tdvmm run demo --ff on --max-virtual-time 1h \
...
`tdvmm.interval` (180s here) that fast-forward collapses; raise it toward `3600`
for genuine hour-apart spacing over a longer run. (`tdvmm.hc_tick` sets how often
the health ticker runs — real work under fast-forward.)

`tdvmm test demo --scenario guest/stacks/demo/demo.yml --logs-dir /tmp/demo-logs`
...
tdvmm runs a **subset** of Compose — the part that fits a closed, single-machine
