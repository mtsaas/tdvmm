...
pub(super) fn write_stack_lock(
    ...
    for r in records {
        match r.policy.as_str() {
            ...
            "squash" => {
                ...
                prov.push(format!(
                    "image  policy=squash  upstream={}  seed_tag=localhost/tdvmm-{base}-{short}:baked  src_diffid={}  size_mib={}  GATE=ok",
                    r.upstream, r.content_id, r.size_mib
                ));
            }
            "build" => {
    ...
    out.push_str("# tdvmm Phase-2a stack manifest (generated by bake-stack.sh).\n");
    ...
    out.push_str(&format!("agent_sha256         {agent_sha}  tdvmm-agent\n"));
