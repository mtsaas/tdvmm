//! The Rust compose pipeline for `dvmm build` (OP-1b) — a faithful in-binary port
...
//!     reject everything outside it with the same loud `DVMM_BAKE_REJECT:`
...
//!
//! The OP-1b gate is a **byte-identical** `.dvmm` versus the old script producer,
//! and `compose.lock.yml` is embedded in BOTH the initramfs and the `.dvmm`. The
...
pub const REJECT: &str = "DVMM_BAKE_REJECT";
pub const WARN: &str = "DVMM_BAKE_WARN";
...
fn build_output_tag(sname: &str, scfg: &serde_yaml::Mapping) -> String {
    ...
    let safe: String = sname
        ...
        .collect();
    format!("localhost/dvmm-build-{safe}:baked")
}
...
/// the retired `bake_compose.py` so the `.dvmm` gate holds).
