...
// SocketPath is the default control socket the in-guest tdvmm-agent serves,
// bind-mounted into every container. Mirrors tdvmm_proto::CONTROL_SOCKET_PATH.
// Override it per-process with TDVMM_CONTROL_SOCKET, or per-call with Dial.
const SocketPath = "/run/tdvmm/ctl/sock"

// EnvSocketPath overrides SocketPath for Connect.
...
// DefaultRetry bounds how long Connect keeps retrying the initial dial.
...
// CommandError is returned when the agent refused a command (a reply with
// ok=false).
type CommandError struct {
	...
	// e.g. "no_container", "nft", "podman_op", "unknown_op".
	Code string
}

func (e *CommandError) Error() string { return e.Op + ": " + e.Reason }

// codeOf returns the prefix of an agent error string before the first ':', or "".
...
// Request is one command line sent to the agent. Optional fields are omitted
// from the wire line when unset. Id is assigned by the client on send.
type Request struct {
	Id        uint64   `json:"id"`
	Op        string   `json:"op"`
...
// GuestEvent is a guest→host event. A control-socket client never receives one;
// it is part of the Reply type.
...
// encodeLine renders one message as a framed wire line: compact JSON plus a
...
// verdict (Finish). Create one with Connect or Dial. Safe for concurrent use:
// each command is a serialized request/reply round-trip on the one socket.
type Client struct {
	mu       sync.Mutex
	conn     net.Conn
	r        *bufio.Reader
...
// TDVMM_CONTROL_SOCKET override), retrying briefly while the agent binds it. It
// errors if the socket never appears.
...
// Dial connects once to a specific control-socket path (no retry).
...
// Do sends one command and returns the agent's reply. A reply with ok=false is
// returned as a *CommandError.
...
// once the rule is installed. Services are compose service names.
func (c *Client) Partition(a, b string) error {
	_, err := c.Do(&Request{Op: "partition", Container: strPtr(a), Peer: strPtr(b)})
	return err
}

// Heal undoes one partition, or ALL of them when called with no arguments. Any
// argument count other than 0 or 2 is an error.
...
// Kill SIGKILLs a service's container and waits for it to be dead.
...
// Running returns the set of services with a running container, as a map for
...
// Exec runs a command inside ANOTHER service's container (argv, exec'd directly).
// A nonzero exit is a result in the reply, not an error; only the agent failing
// to run the command returns an error.
...
// See Exec for the exit semantics.
...
// them. A pred that panics is treated as "not yet". It errors on timeout.
...
// Finish ends the run with a verdict: 0 passes, anything else fails. It blocks
// until the agent acknowledges; the harness tears the VM down immediately after,
// so treat it as the last statement the driver runs. It does NOT close the
// socket. The FIRST finish decides the run; a second is refused with a
// *CommandError. Pass an empty message to omit it.
